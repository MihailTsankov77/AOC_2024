...
check_is_cycle grid init_pos rotation_count pos dim rotation =
  trace (show pos ++ " " ++ show init_pos ++ " " ++ show rotation_count) $
    if rotation_count == 4
      then init_pos == pos
      else
        ( if is_outside dim pos
            then False
              ...
              ( if next_value == 1
                  then check_is_cycle grid init_pos (rotation_count + 1) pos dim (rotate rotation)
                  else check_is_cycle grid init_pos rotation_count next_pos dim rotation
              )
        )
  where
    next_pos = get_movement rotation pos
...
consume_part2 grid pos dim rotation count visited =
  ...
  where
    ...
    was_visited = lookupWithDefault False next_pos visited

    count' =
      if was_visited
        && (check_is_cycle (HashMap.insert (get_movement rotation next_pos) 1 grid) next_pos 0 next_pos dim (rotate rotation))
        then count + 1
        else count
